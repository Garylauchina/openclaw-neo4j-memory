...
class MeditationLLMClient:
    ...
    def distill_knowledge(
        ...
        """从密集子图中蒸馏元知识。"""
        system_prompt = (
            "分析以下局部子图中的实体及其关系，提取出仅关于该局部子图的高层次洞察或规律。\n"
            "这是一次局部蒸馏，不是全局系统总结。不要把结论泛化为整个系统、整个领域或宏大范式。\n"
            "总结必须严格受输入子图支持，只能概括核心实体及其邻域中明确可见的模式。\n"
            "输出必须保留局部身份：summary 中必须显式包含核心实体名称，且尽量点名 1-3 个相关实体名称。\n"
            "避免使用过于空泛的抽象表达，如‘复杂系统的核心规律’、‘范式转移’、‘全局治理逻辑’，除非这些词就在输入中并被直接支持。\n"
            "如果输入过于稀疏或不足以形成局部高阶规律，请降低 confidence，而不要强行上升抽象。\n"
            "用一到两句话概括，要求信息密度高，但必须可追溯到输入子图。\n"
            "只返回 JSON 格式：\n"
            '{"summary": "局部蒸馏元知识", "confidence": 0.0-1.0}\n'
            "只返回 JSON，不要其他文字。"
        )
        edge_descs = [
        ...
        user_prompt = (
            ...
            f"关系:\n" + "\n".join(edge_descs[:20]) + "\n\n"
            "要求：请只总结这个核心实体及其局部邻域，不要扩展到输入之外的系统级规律。"
        )
        try:
            resp = self._call_llm(system_prompt, user_prompt)
            result = self._parse_json_response(resp)

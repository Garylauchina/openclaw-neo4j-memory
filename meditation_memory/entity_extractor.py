...
class Entity:
    ...
    name: str  # 实体名称
    entity_type: str  # 类型：person / place / concept / event / intent / object / organization
...
class Relation:
    ...
    source: str  # 源实体名称
    target: str  # 目标实体名称
    relation_type: str  # 关系类型，如 located_in / works_at / related_to / causes
...
要求：
1. 实体类型包括：person（人物）、place（地点）、concept（概念）、event（事件）、
   intent（意图/目标）、object（物品）、organization（组织）
2. 关系描述实体之间的联系，尽量使用具体的关系类型：
   ...
   减少使用模糊的 related_to
3. 特别注意提取：
   - 事件（event）：对话中提到的动态行为或发生的事情
   - 意图（intent）：用户的目标、期望或计划
   - 因果关系：事件之间的因果链条
4. 质量约束：
   - 只提取有意义的命名实体和核心概念
   ...
   - 实体名称必须是完整的词或短语
   - 不要提取代词、连词、副词等虚词
5. 限制：每次最多提取 15 个最重要的实体
6. 只抽取文本中明确提到或可直接推断的信息，不要编造
